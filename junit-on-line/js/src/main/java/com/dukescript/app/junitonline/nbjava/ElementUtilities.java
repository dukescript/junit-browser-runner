...
public final class ElementUtilities {
    ...
    ElementUtilities(final CompilationInfo info) {
        this((JavacTaskImpl)info.getJavacTask(),info);
    }

    ElementUtilities(final JavacTaskImpl jt) {
        this(jt,null);
    }
    ...
    public Element getImplementationOf(ExecutableElement method, TypeElement origin) {
        return ((MethodSymbol)method).implementation((TypeSymbol)origin, com.sun.tools.javac.code.Types.instance(ctx), true);
    }
    ...
    public boolean overridesMethod(ExecutableElement element) {
        ...
        if ((m.flags() & Flags.STATIC) == 0) {
            ...
            com.sun.tools.javac.code.Types jctypes = com.sun.tools.javac.code.Types.instance(ctx);
            for (Type sup = jctypes.supertype(m.owner.type);
                 sup.hasTag(TypeTag.CLASS);
                 sup = jctypes.supertype(sup)) {
                for (com.sun.tools.javac.code.Scope.Entry e = sup.tsym.members().lookup(m.name);
...
//        return (doc instanceof JavadocEnv.ElementHolder) ? ((JavadocEnv.ElementHolder)doc).getElement() : null;
    ...
    public boolean alreadyDefinedIn(CharSequence name, ExecutableType method, TypeElement enclClass) {
        ...
        com.sun.tools.javac.code.Scope scope = clazz.members();
        Name n = Names.instance(ctx).fromString(name.toString());
        Types types = JavacTypes.instance(ctx);
        scanSymbol:
        for(com.sun.tools.javac.code.Scope.Entry e = scope.lookup(n); e.scope==scope; e = e.next())
    ...
    public boolean isMemberOf(Element e, TypeElement type) {
        return ((Symbol)e).isMemberOf((TypeSymbol)type, com.sun.tools.javac.code.Types.instance(ctx));
    }
	...
	for (Type t = jctypes.supertype(origin.type); t.hasTag(TypeTag.CLASS); t = jctypes.supertype(t)) {
	    TypeSymbol c = t.tsym;
	    com.sun.tools.javac.code.Scope.Entry e = c.members().lookup(m.name);
	    while (e.scope != null) {
	...
	for (Type type : jctypes.interfaces(m.owner.type)) {
	    for (com.sun.tools.javac.code.Scope.Entry e = type.tsym.members().lookup(m.name);
		 e.scope != null; e = e.next()) {
		if (m.overrides(e.sym, owner, jctypes, true))
		    return true;
	    }
     ...
     */
    public boolean isErroneous(Element e) {
        if (e == null) {
            return true;
        }
